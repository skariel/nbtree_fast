...
function issorted(itr, order::Ordering)
    ...
    while !done(itr, state)
        ...
        lt(order, this.x, prev.x) && return false
        prev = this
    end
    return true
end
...
@inline function selectpivot!(v::AbstractVector, lo::Int, hi::Int, o::Ordering)
    @inbounds begin
        ...
        if lt(o, v[mi].x, v[lo].x)
            v[mi], v[lo] = v[lo], v[mi]
        end
        if lt(o, v[hi].x, v[mi].x)
            if lt(o, v[hi].x, v[lo].x)
...
function partition!(v::AbstractVector, lo::Int, hi::Int, o::Ordering)
    ...
    @inbounds while true
        i += 1; j -= 1
        while lt(o, v[i].x, pivot.x); i += 1; end;
        while lt(o, pivot.x, v[j].x); j -= 1; end;
        i >= j && break
        v[i], v[j] = v[j], v[i]
    end
    v[j], v[lo] = pivot, v[j]
...
issignleft(o::ForwardOrdering, x::Floats) = lt(o, x.x, 0.0)
issignleft(o::ReverseOrdering, x::Floats) = lt(o, x.x, -0.0)
